    ...
    function date() {
        ...
        return {
            validate: function (input) {
                ...
                if (timeFormat) {
                    var klinik = timeSection.split(':');
                    if (timeFormat.split(':').length !== klinik.length) {
                        return invalidResult;
                    }
                    var h = klinik.length > 0 ? (klinik[0].length <= 2 && /^\d+$/.test(klinik[0]) ? parseInt(klinik[0], 10) : -1) : 0;
                    var m = klinik.length > 1 ? (klinik[1].length <= 2 && /^\d+$/.test(klinik[1]) ? parseInt(klinik[1], 10) : -1) : 0;
                    var s = klinik.length > 2 ? (klinik[2].length <= 2 && /^\d+$/.test(klinik[2]) ? parseInt(klinik[2], 10) : -1) : 0;
                    if (h === -1 || m === -1 || s === -1) {
                        return invalidResult;
                    }
                    if (s < 0 || s > 60) {
