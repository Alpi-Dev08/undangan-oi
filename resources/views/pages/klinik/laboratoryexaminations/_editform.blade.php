    ...
    <div class="card-body pb-0">
        ...
        <div class="tab-content">
            ...
            <div class="tab-pane active" id="examination" role="tabpanel" aria-labelledby="all-tab" data-kt-timeline-widget-4-blockui="true">
                <form id="kt_modal_add_laboratoryexaminations_form" enctype="multipart/form-data" method="POST" class="form" action="{{ route('laboratoryexaminations.update',['laboratoryexamination' => $laboratoryexamination->id]) }}">
                {{ csrf_field() }}
                @method('PUT')
                <!--begin::Scroll-->
                    <div class="d-flex flex-column flex-row-fluid">
                            ...
                            <div class="col-lg-8">
                                ...
                                <select name="laboratory_id" aria-label="{{ __('Laboratory') }}" data-control="select2" data-placeholder="{{ __('Select a Laboratory...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                                    ...
                                    @foreach($laboratories as $laboratory)
                                        <option {{ $laboratory->id === old('laboratory_id', $laboratoryexamination->laboratory_id ?? '') ? 'selected' :'' }} value="{{ $laboratory->id }}">{{  $laboratory->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-6">
                            <!--begin::Label-->
                            <label class="col-lg-4 col-form-label required fw-bold fs-6">{{ __('Ref Doctor') }}</label>
                            <!--end::Label-->
                            <!--begin::Col-->
                            <div class="col-lg-8">
                                ...
                                <input type="text" name="other_health_profesional" class="form-control form-control-lg form-control-solid fw-bold mt-5 border border-gray-300" placeholder="{{ __('Other Ref Doctor') }}" value=" {{ $examination->health_profesional_id ? '' : $laboratoryexamination->ref_dokter }}" />
                            </div>
                        </div>
                        ...
                        <div class="row mb-6">
                            <!--begin::Label-->
                            <label class="col-lg-4 col-form-label required fw-bold fs-6">{{ __('Service Type') }}</label>
                            <!--end::Label-->
                            <!--begin::Col-->
                            <div class="col-lg-8">
                                <select name="service_type_id" required aria-label="{{ __('Service Type') }}" data-control="select2" data-placeholder="{{ __('Select a Service Type...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                                    <option value="">{{ __('Select a Service Type...') }}</option>
                                    @foreach($services as $service)
                                        <option {{ $service->id === old('service_id', $examination->service_type_id ?? '') ? 'selected' :'' }} value="{{ $service->id }}" >{{  $service->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row mb-6">
                            ...
                            <!--begin::Col-->
                            <div class="col-lg-8">
                                <select name="package_id" aria-label="{{ __('Package') }}" data-control="select2" data-placeholder="{{ __('Select a Package...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                                    <option value="">{{ __('Select a Package...') }}</option>
                                    @foreach($packages as $package)
                                        <option {{ $package->id === old('package_id', $examination->package_id ?? '') ? 'selected' :'' }} value="{{ $package->id }}"> {{  $package->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-8">
                                <select name="laboratory_unit_id" aria-label="{{ __('Laboratory Unit') }}" data-control="select2" data-placeholder="{{ __('Select a Laboratory Unit...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                                    ...
                                    @foreach($laboratory_units as $laboratory_unit)
                                        <option {{ $laboratory_unit->id === old('laboratory_unit_id', $laboratoryexamination->laboratory_unit_id ?? '') ? 'selected' :'' }}  value="{{ $laboratory_unit->id }}">{{  $laboratory_unit->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                                            ...
                                            <ul>
                                                @foreach(types($category->id) as $type)
                                                    <li style="list-style: none;">
                                                        <div class="form-check form-check-custom form-check-solid mb-3">
                                                            <input class="form-check-input" {{ in_array($type->id,$types) ? 'checked' : '' }} type="checkbox" value="{{ $type->id }}" name="types[]" id="type_{{ $type->id }}">
                        ...
                        <div class="row mb-6">    <!--begin::Input group-->
                            <!--begin::Label-->
                            <label class="col-lg-4 col-form-label fw-bold fs-6">Upload Result</label>
                            <!--end::Label-->
                            <!--begin::Input-->
                            <div class="col-lg-8 pt-5">
                                <div class="fv-row mb-7">
                                    <div class="input-group input-group-solid mb-3 fv-row">
                                        <input type="file" name="file" accept=".pdf" class="required col-lg-10 form-control form-control-solid mb-3 mb-lg-0 @error('file') is-invalid @enderror" placeholder="File Encryption name" value=""/>
                                        <button type="submit" class="btn btn-primary col-lg-2" data-kt-filemanagers-modal-action="submit">
                        ...
                        <div class="text-center">
                            <a href="{{ route('laboratoryexaminations.index')  }}" class="btn btn-sm btn-light-primary">
