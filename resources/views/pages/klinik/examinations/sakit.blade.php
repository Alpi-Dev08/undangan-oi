...
<body style="font-family: 'Nunito Sans', sans-serif;">
    ...
    <main style="font-size:13px!important;">
        <p
            style="color:#000;margin:0px;font-size:16px;text-align:center;font-weight:bolder;text-transform:uppercase;font-family: 'Roboto Condensed', sans-serif;margin-top:20px;text-decoration:underline;">
            Surat Keterangan Berobat/Sakit</p>
        <p
            style="color:#000;margin:0px;font-size:16px;text-align:center;font-weight:bolder;text-transform:uppercase;font-family: 'Roboto Condensed', sans-serif;margin-bottom:50px;text-decoration:underline;">
            ATTENDANCE / ILLNESS CERTIFICATE</p>
                ...
                <tbody>
                    <tr>
                        <td style="vertical-align:top"><input type="checkbox"
                                {{ $data->keterangan == 1 ? 'checked' : '' }}></td>
                        <td style="padding-bottom: 15px">Dapat kembali bekerja <br><em>May return to work</em></td>
                    </tr>
                    <tr>
                        <td style="vertical-align:top"><input type="checkbox"
                                {{ $data->keterangan == 2 ? 'checked' : '' }}></td>
                        @if ($data->keterangan == 2)
                            <td style="padding-bottom: 15px">
                                Disarankan untuk beristirahat selama : <b>{{ $data->hari ?? '0' }}</b> hari, dari
                                <b>{{ $data->start_date ? \Carbon\Carbon::parse($data->start_date)->locale('id')->format('d F Y') : '-' }}</b>
                                s.d
                                <b>{{ $data->end_date ? \Carbon\Carbon::parse($data->end_date)->locale('id')->format('d F Y') : '-' }}</b><br>
                                <em>Should have a complete rest for : <b>{{ $data->hari ?? '0' }}</b> day (s), from
                                    <b>{{ $data->start_date ? \Carbon\Carbon::parse($data->start_date)->locale('id')->format('d F Y') : '-' }}</b>
                                    to
                                    <b>{{ $data->end_date ? \Carbon\Carbon::parse($data->end_date)->locale('id')->format('d F Y') : '-' }}</b></em>
                            </td>
                        @else
                            <td style="padding-bottom: 15px">
                    ...
                    <tr>
                        <td style="vertical-align:top"><input type="checkbox"
                                {{ $data->keterangan == 3 ? 'checked' : '' }}></td>
                        @if ($data->keterangan == 3)
                            <td style="padding-bottom: 15px">Perlu datang kembali ke klinik pada :
                                <b>{{ $data->back_date ? \Carbon\Carbon::parse($data->back_date)->locale('id')->format('d F Y') : '-' }}</b><br>
                                <em>Need to be seen again at clinic on :
                                    <b>{{ $data->back_date ? \Carbon\Carbon::parse($data->back_date)->locale('id')->format('d F Y') : '-' }}</b></em>
                            </td>
                        @else
                            <td style="padding-bottom: 15px">Perlu datang kembali ke klinik pada : <b>-</b><br>
                                <em>Need to be seen again at clinic on : <b>-</b></em>
                            </td>
                        @endif
                    </tr>
                    <tr>
                        <td style="vertical-align:top"><input type="checkbox"
                                {{ $data->keterangan == 4 ? 'checked' : '' }}></td>
                        <td style="padding-bottom: 15px">Perlu dirujuk ke Rumah Sakit untuk mendapatkan pemeriksaan
                            lebih lanjut<br><em>Need to be referred to hospital for further treatment</em></td>
                    </tr>
                </tbody>
            </table>
        @endif
