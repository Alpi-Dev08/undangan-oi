    ...
    <table style="width:100%;border-bottom-width:2px;border-bottom-style:solid">
        <tr style="vertical-align:baseline">
            <td colspan="2" style="width: 100%;text-align:center">
                <p style="margin:0px;font-size:22px;text-align:center;color:gray;font-weight:bolder;text-transform:uppercase;font-family: 'Roboto Condensed', sans-serif;">
                    Medical Record</p>
            </td>
        </tr>
    </table>
        ...
        <tr>
            ...
            <td style="width:50%;text-align: right;vertical-align: bottom;float: right;height:100px">
                <img src="{{ public_path(theme()->getMediaUrlPath() . 'logos/qr.jpeg') }}"
                    ...
                    src="{{ public_path(theme()->getMediaUrlPath() . 'logos/logo-yayasan.png') }}" style="height:75px;">
            </td>
        </tr>
    </table>
</footer>
<main>
    <div>
        <h4 style="font-weight: bolder;color:#436ba4;font-size:12px;margin:0px;margin-top:10px;text-transform: uppercase">
            Vital Sign & BMI</h4>
        ...
        @if($examination->service_category->is_mcu == 1)
            ...
            @if(isset($examination->anamnesis->anamnesis_value))
                ...
                <table style="width:100%;font-size:12px;">
                    <tr>
                        <td colspan="2" style="padding-left:15px;font-weight:bold">Present Complaint / Keluhan Saat
                            Ini
                        </td>
                    </tr>
                    <tr>
                    ...
                    @foreach($anamnesis as $key => $value)
                        ...
                        @if($radio && $additional[$additionalKeys[0]])
                            @if($header != getAnamnesis($key)->anamnesis_category_id)
                                <tr>
                                    <td colspan="2"
                                        style="padding-left:15px;font-weight:bold">{{ getAnamnesisCategory(getAnamnesis($key)->anamnesis_category_id)->name }}</td>
                                </tr>
                            ...
                            <tr>
                                ...
                                <td style="width:65%;">
                                    : {{ ucwords($radio[$radioKeys[0]]).', '.$additional[$additionalKeys[0]] }}</td>
                            </tr>
                        @elseif($radio)
                            @if($header != getAnamnesis($key)->anamnesis_category_id)
                                <tr>
                    ...
                    @foreach($physicals as $key => $value)
                        ...
                        @if($radio && $additional[$additionalKeys[0]])
                            @if($header != getPhysicals($key)->physical_category_id)
                                <tr>
                                    <td colspan="2"
                                        style="padding-left:15px;font-weight:bold">{{ getPhysicalsCategory(getPhysicals($key)->physical_category_id)->name }}</td>
                                </tr>
                        ...
                        @if($header != getPhysicals($key)->physical_category_id)
                            <tr>
                                <td style="width:35%;padding-left:15px;"
                                    colspan="2">{{ getPhysicalsCategory(getPhysicals($key)->physical_category_id)->name }}</td>
                                @php $header = getPhysicals($key)->physical_category_id; @endphp
                            </tr>
                        @endif
                            ...
                            @if($header != getAdditional($key)->additionals_category_id)
                                <tr>
                                    <td style="width:35%;padding-left:15px;font-weight:bold;padding-top:10px"
                                        colspan="2">{{ getAdditionalCategory(getAdditional($key)->additionals_category_id)->name }}</td>
