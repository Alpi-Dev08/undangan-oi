<form id="kt_modal_add_locations_form" method="POST" class="form" action="{{ route('locations.update',['location' => $location->id]) }}">
    @method('PUT')
    {{ csrf_field() }}
    <!--begin::Scroll-->
    <div class="d-flex flex-column flex-row-fluid">
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="text" name="name" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('name') is-invalid @enderror" placeholder="Location name" value="{{ $location->name ?? '' }}"/>
            </div>
            @error('name')
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="text" name="code" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('code') is-invalid @enderror" placeholder="Location code" value="{{ $location->code ?? '' }}"/>
            </div>
            @error('code')
            <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="email" name="email" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('email') is-invalid @enderror" placeholder="Email" value="{{ $location->email ?? '' }}"/>
            </div>
            @error('email')
            <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="text" name="phone" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('phone') is-invalid @enderror" placeholder="Phone" value="{{ $location->phone ?? '' }}"/>
            </div>
            @error('phone')
            <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="text" name="fax" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('fax') is-invalid @enderror" placeholder="Phone" value="{{ $location->fax ?? '' }}"/>
            </div>
            @error('fax')
            <div class="text-danger">{{ $message }}</div>
            @enderror
            ...
            <div class="input-group input-group-solid has-validation mb-3">
                <select name="country_id" aria-label="{{ __('Select a Country') }}" data-control="select2" data-placeholder="{{ __('Select a country...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @foreach($countries as $country)
                        <option value="{{ $country->id }}" {{  $country->id === old('country_id', $location->country_id ?? '') ? 'selected' :'' }}>{{  $country->name }}</option>
                    @endforeach
                </select>
            </div>
            <!--end::Col-->
                ...
                <select id="province" name="province_id" aria-label="{{ __('Select a Province') }}" data-control="select2" data-placeholder="{{ __('Select a province...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @if(isset($provinces))
                        @foreach($provinces as $province)
                            <option value="{{ $province->id }}" {{ $province->id === old('province_id', $location->province_id ?? '') ? 'selected' :'' }}>{{ $province->name }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
                ...
                <select id="city" name="city_id" aria-label="{{ __('Select a City') }}" data-control="select2" data-placeholder="{{ __('Select a city...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @if(isset($cities))
                        @foreach($cities as $city)
                            <option value="{{ $city->id }}" {{ $city->id === old('city_id', $location->city_id ?? '') ? 'selected' :'' }}>{{ $city->name }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
                ...
                <select id="district" name="district_id" aria-label="{{ __('Select a District') }}" data-control="select2" data-placeholder="{{ __('Select a district...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @if(isset($districts))
                        @foreach($districts as $key => $value)
                            <option value="{{ $value['id'] }}" {{ $value['id'] === old('district_id', $location->district_id ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
                ...
                <select id="subdistrict" name="sub_district_id" aria-label="{{ __('Select a Sub District') }}" data-control="select2" data-placeholder="{{ __('Select a Sub District...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @if(isset($subdistricts))
                        @foreach($subdistricts as $key => $value)
                            <option value="{{ $value['id'] }}" {{ $value['id'] === old('sub_district_id', $location->sub_district_id ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="text" name="address" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('address') is-invalid @enderror" placeholder="Address" value="{{ $location->address ?? '' }}"/>
            </div>
            @error('address')
            <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="text" name="postal_code" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('postal_code') is-invalid @enderror" placeholder="Popstal Code" value="{{ $location->postal_code ?? '' }}"/>
            </div>
            @error('postal_code')
            <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <textarea name="description" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('description') is-invalid @enderror" placeholder="Description">{{ $location->description ?? '' }}</textarea>
            </div>
            @error('description')
            <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="fv-row mb-7">
            ...
            <label class="form-check form-switch form-check-custom form-check-solid">
                <input class="form-check-input" name="status" type="checkbox" value="1" {{ $location->status==1 ? 'checked="checked"' : '' }} />
