<form id="kt_modal_add_subdistricts_form" method="POST" class="form" action="{{ route('subdistricts.update',['subdistrict' => $subdistrict->id]) }}">
@method('PUT')
{{ csrf_field() }}
<!--begin::Scroll-->
    <div class="d-flex flex-column flex-row-fluid">
            ...
            <div class="input-group input-group-solid has-validation mb-3">
                <select name="country_id" aria-label="{{ __('Select a Country') }}" data-control="select2" data-placeholder="{{ __('Select a country...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @foreach($country as $key => $value)
                        <option value="{{ $value['id'] }}" {{ $value['id'] === old('country', $subdistrict->country_id ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                    @endforeach
                </select>
            </div>
            @error('country_id')
                ...
                <select id="province" name="province_id" aria-label="{{ __('Select a Province') }}" data-control="select2" data-placeholder="{{ __('Select a province...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @if(isset($province))
                        @foreach($province as $key => $value)
                            <option value="{{ $value['id'] }}" {{ $value['id'] === old('province', $subdistrict->province_id ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
                ...
                <select id="city" name="city_id" aria-label="{{ __('Select a City') }}" data-control="select2" data-placeholder="{{ __('Select a city...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @if(isset($city))
                        @foreach($city as $key => $value)
                            <option value="{{ $value['id'] }}" {{ $value['id'] === old('city', $subdistrict->city_id ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
                ...
                <select id="district" name="district_id" aria-label="{{ __('Select a District') }}" data-control="select2" data-placeholder="{{ __('Select a district...') }}" class="form-select form-select-solid form-select-lg fw-bold">
                    ...
                    @if(isset($district))
                        @foreach($district as $key => $value)
                            <option value="{{ $value['id'] }}" {{ $value['id'] === old('district', $subdistrict->district_id ?? '') ? 'selected' :'' }}>{{ $value['name'] }}</option>
                        @endforeach
                    @endif
                </select>
            </div>
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="text" name="area_code" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('area_code') is-invalid @enderror" placeholder="Sub District Name" value="{{ $subdistrict->area_code ?? '' }}"/>
            </div>
            @error('area_code')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="text" name="name" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('name') is-invalid @enderror" placeholder="Sub District name" value="{{ $subdistrict->name ?? '' }}"/>
            </div>
            @error('name')
            <div class="text-danger">{{ $message }}</div>
        @enderror
        ...
        <div class="fv-row mb-7">
            ...
            <!--begin::Input-->
            <div class="input-group input-group-solid has-validation mb-3">
                <input type="number" name="postal_code" class="form-control form-control-solid border border-gray-300 mb-3 mb-lg-0 @error('postal_code') is-invalid @enderror" placeholder="Sub District name" value="{{ $subdistrict->postal_code ?? '' }}"/>
            </div>
            @error('postal_code')
            <div class="text-danger">{{ $message }}</div>
        @enderror
